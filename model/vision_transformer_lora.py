...
class Block(nn.Module):
    ...
    def __init__(
        ...
        Args:
            ...
            use_lora: If True, apply LoRA to query and key projections.
            lora_r: Rank for LoRA decomposition.
        ...
        # Add LoRA to query and key projections if requested
        self.use_lora = use_lora
        if use_lora and hasattr(self.attn, 'qkv'):
            # Store original qkv layer
            original_qkv = self.attn.qkv
            
            # Create separate LoRA layers for query and key
            # Query LoRA: input_dim -> output_dim (same as qkv input/output dims)
            self.lora_q_A = nn.Parameter(torch.zeros(dim, lora_r, **dd))
            self.lora_q_B = nn.Parameter(torch.zeros(lora_r, dim, **dd))
            
            # Key LoRA: input_dim -> output_dim
            self.lora_k_A = nn.Parameter(torch.zeros(dim, lora_r, **dd))
            self.lora_k_B = nn.Parameter(torch.zeros(lora_r, dim, **dd))
            ...
            for param in original_qkv.parameters():
                param.requires_grad = False
    ...
    def forward(self, x: torch.Tensor, attn_mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        if self.use_lora and hasattr(self.attn, 'qkv'):
            # Custom forward with LoRA for query and key
            x_norm = self.norm1(x)
            
            # Get qkv from base attention layer
            B, N, C = x_norm.shape
            qkv = self.attn.qkv(x_norm)
            
            # Apply LoRA to query and key portions
            x_lora = self.lora_dropout(x_norm)
            
            # Compute LoRA adaptations for query and key
            lora_q = (x_lora @ self.lora_q_A @ self.lora_q_B) * self.lora_scaling
            lora_k = (x_lora @ self.lora_k_A @ self.lora_k_B) * self.lora_scaling
            
            # Reshape qkv and add LoRA to query and key
            qkv = qkv.reshape(B, N, 3, C)
            qkv[:, :, 0, :] = qkv[:, :, 0, :] + lora_q  # Add to query
            qkv[:, :, 1, :] = qkv[:, :, 1, :] + lora_k  # Add to key
            qkv = qkv.reshape(B, N, 3 * C)
            
            # Temporarily replace qkv output by modifying the attention computation
            # We need to call the rest of attention forward pass manually

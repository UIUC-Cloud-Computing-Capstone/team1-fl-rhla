...
class ViTBlock(nn.Module):
    def __init__(self, num_hiddens, norm_shape, mlp_num_hiddens,
                 num_heads, dropout, use_bias=False, use_lora=True,
...
class ViT(Classifier):
    """Vision Transformer with LoRA support."""
    def __init__(self, img_size, patch_size, num_hiddens, mlp_num_hiddens,
                 num_heads, num_blks, emb_dropout, blk_dropout, lr=0.1,
                 use_bias=False, num_classes=10, use_lora=True, 
                 lora_r=8, lora_alpha=16, lora_dropout=0.1):
        super().__init__()
        self.save_hyperparameters()
        ...
        for i in range(num_blks):
            self.blks.add_module(f"{i}", ViTBlock(
                ...
                num_heads, blk_dropout, use_bias, use_lora=use_lora,
    ...
    def get_lora_parameters(self):
        """Get only the LoRA parameters (trainable low-rank matrices)."""
        lora_params = []
...
# Example: Training Vision Transformer with LoRA
if __name__ == "__main__":
    img_size, patch_size = 96, 16
    num_hiddens, mlp_num_hiddens, num_heads, num_blks = 512, 2048, 8, 2
    emb_dropout, blk_dropout, lr = 0.1, 0.1, 0.1
    
    # Create model with LoRA enabled (default)
    # LoRA parameters: r=8, alpha=16, dropout=0.1
    model = ViT(img_size, patch_size, num_hiddens, mlp_num_hiddens, num_heads,
                num_blks, emb_dropout, blk_dropout, lr, use_lora=True, 
                lora_r=8, lora_alpha=16, lora_dropout=0.1)
    
    # Initialize the model with dummy data to materialize lazy layers
    ...
    for name, param in lora_params[:4]:  # Show first 4
